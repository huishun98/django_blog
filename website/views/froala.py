...
from .db_wrappers import get_s3_conn
...
def upload_file(request):
    ...
    try:
        response = File.upload(DjangoAdapter(
            request), settings.MEDIA_URL, options)
    except Exception:
...
class File(object):
    ...
    def upload(req, fileRoute, options=None):
        ...
        try:
            sha1 = hashlib.sha1(str(time.time()).encode(
                "utf-8")).hexdigest()  # v3
        except Exception:
        ...
        req.saveFile(options["fieldname"], sha1 + extension)
        ...
        if "resize" in options and options["resize"] is not None:
            with WandImage(filename=fullNamePath) as img:
...
def upload_image(request):
    try:
        response = Image.upload(DjangoAdapter(request), settings.MEDIA_URL)
    except Exception:
        response = {"error": str(sys.exc_info()[1])}
    return HttpResponse(json.dumps(response), content_type="application/json")
...
def upload_image_validation(request):
    ...
    try:
        response = Image.upload(DjangoAdapter(
            request), settings.MEDIA_URL, options)
    except Exception:
        response = {"error": str(sys.exc_info()[1])}
    return HttpResponse(json.dumps(response), content_type="application/json")
...
class Image(object):
    ...
    def list(folderPath, thumbPath=None):
        ...
        fnames = [f for f in listdir(absoluteFolderPath) if isfile(
...
def upload_video(request):
    try:
        response = Video.upload(DjangoAdapter(request), settings.MEDIA_URL)
    except Exception:
        response = {"error": str(sys.exc_info()[1])}
    return HttpResponse(json.dumps(response), content_type="application/json")
...
def upload_video_validation(request):
    ...
    try:
        response = Video.upload(DjangoAdapter(
            request), settings.MEDIA_URL, options)
    except Exception:
        response = {"error": str(sys.exc_info()[1])}
    return HttpResponse(json.dumps(response), content_type="application/json")
...
class Utils(object):
    ...
    def hmac(key, string, hex=False):
        ...
        # python 2-3 compatible:
        try:
            hmac256 = hmac.new(key.encode() if isinstance(key, str) else key, msg=string.encode(
                "utf-8") if isinstance(string, str) else string, digestmod=hashlib.sha256)  # v3
        except Exception:
...
class DjangoAdapter(BaseAdapter):
    ...
    def saveFile(self, fieldname, filename):
        print("should save now")
        self.checkFile(fieldname)

        if settings.DEBUG:
            fs = FileSystemStorage()
            return fs.save(os.path.join(filename), self.request.FILES[fieldname])

        s3 = get_s3_conn()

        return s3.Bucket(settings.AWS_STORAGE_BUCKET_NAME).put_object(
            Key=filename, Body=self.request.FILES[fieldname])
